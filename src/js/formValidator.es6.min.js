...
class jsFilter{checkStatus(elem){let status;status=!0;if(!1===new jsValidator().forceFilter){status=!1;if(!0===elem.required){status=!0}}
    ...
    this.value=final_value};isAlpha(event){if(!0===helper.isWindowAction(event))return!0;let status=new pattern().validate(event,'a-zA-Z');if(!1===status)event.preventDefault()};isAlphaNumeric(event){if(!0===helper.isWindowAction(event))return!0;let status=new pattern().validate(event,'a-zA-Z0-9');if(!1===status)event.preventDefault()};isValidPassword(event){let charCode=(event.which)?event.which:event.keyCode;if(charCode===32){event.preventDefault();return!1}
    if(!0===helper.isWindowAction(event))return!0;let status=new pattern().validate(event,'a-zA-Z0-9');if(!1===status)event.preventDefault()};isPatternValid(event){if(!0===helper.isWindowAction(event))return!0;let status=new pattern().validate(event,'a-zA-Z0-4');if(!1===status)event.preventDefault()};isNumberKey(event){if(!0===helper.isWindowAction(event))return!0;let charCode=(event.which)?event.which:event.keyCode;if(charCode===46||charCode>31&&(charCode<48||charCode>57)){event.preventDefault();return!1}
    return!0}}
...
class jsField{required(field,forceFilter){let requiredFieldsList=[];for(let i=0;i<field.length;i++){if(field[i].required===!0||!0===forceFilter){requiredFieldsList.push(field[i])}}
    return requiredFieldsList}}
class jsRuleSets{checkValidation(activeElem,log){let validElem=!0;let jsRuleSets_obj=new jsRuleSets();if(!jsRuleSets_obj.isSet(activeElem)){log.push({'el':activeElem,'type':'required','id':activeElem.name+__err_id_suffix_rand_hash});validElem=!1}
    ...
    if(!0===validElem){if(activeElem.name!==''){let elem=document.getElementById(activeElem.name+__err_id_suffix_rand_hash);if(typeof(elem)!=='undefined'&&elem!==null){elem.remove()}}}
    if(!1!==validElem){}
    return log};isSet(elem){if(!1===elem.required)return!0;let status=!0;let value=elem.value;if(value.length===0||value===''||value===' '||value==='[]')status=!1;return status};min(elem){if(!1===elem.required)return!0;let status=!0;let value=elem.value;let min=elem.min;if(value.length<min&&value.length!=0)status=!1;return status};max(elem){if(!1===elem.required)return!0;let status=!0;let value=elem.value;let max=elem.max;if(value.length>max&&value.length!=0)status=!1;return status};email(elem){if(!1===elem.required)return!0;let status=!1;let email=elem.value;if(typeof email==='undefined')return!1;email=email.toString();if(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)){status=!0}
